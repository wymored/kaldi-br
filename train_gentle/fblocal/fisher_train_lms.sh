...
for f in data/train/text data/test/text $lexicon ; do
...
cat data/{train,test}/text | awk -v lex=$lexicon 'BEGIN{while((getline<lex) >0){ seen[$1]=1; } } 
...
cat $dir/unigram.counts | awk '{print $2}' | \
  get_word_map.pl "<s>" "</s>" "<UNK>" > $dir/word_map || exit 1;
...
ln -sv $(readlink -f $dir/3gram-mincount/lm_unpruned.gz) $dir/lm_tglarge.arpa.gz
