...
if [ $stage -le -1 ]; then
  ...
  echo "[$(date +'%F %T')] $0: linking local data" | lolcat
  fblocal/link_local_data.sh --nj 6 $HOME/fb-gitlab/fb-audio-corpora $data || exit 1
fi
...
if [ $stage -le 1 ]; then
  ...
  fblocal/prep_data.sh --nj 6 --test-dir lapsbm16k $data ./data
...
## Train a chain model
## NOTE: CB: if you do not have an NVIDIA card, just give up.
##       OTOH, if you have multiple NVIDIA GPUs, then you might 
##       want to increase the number of jobs final accordingly
#if [ $stage -le 9 ]; then
#  echo "[$(date +'%F %T')] $0: run TDNN script" | lolcat
#  fblocal/chain/run_tdnn.sh --num-epochs 20 \
#      --jobs-initial 1 --jobs-final 1
#fi
